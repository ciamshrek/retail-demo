...
async function registerDynamicTool(
  ...
  try {
    ...
    mcpServerInstance.registerTool(
      toolName,
      toolDefinition.description || `Tool ${toolDefinition.name} from ${proxyClient.getServiceConfig().name}`,
      inputSchemaAsZod,
      outputSchemaAsZod,
      async (args: any, extra: any) => {
        try {
          ...
          return {
            content: [{ 
              type: 'text', 
              text: `Error calling ${toolDefinition.name}: ${getErrorMessage(error)}` 
            }]
          };
        }
      }
    ...
    console.info(`[Discovery] Tool registered: ${toolName} (from ${proxyClient.getServiceConfig().name})`);
    return true;
  } catch (error) {
...
const handler = createMcpHandler(
  (server) => {
    ...
    server.tool(
      ...
      {
        buyerTag: z.string().describe('Buyer internal transaction ID for tracking'),
        sellerServiceId: z.string().describe('Target seller service ID to create token for'),
      },
      async ({ buyerTag, sellerServiceId }) => {
        try {
          ...
          return {
            content: [
              { type: 'text', text: `Error creating KYA token: ${errorMessage}` }
            ],
          }
        }
      }
    ...
    server.tool(
      ...
      async ({ buyerTag, sellerServiceId, amount }) => {
        try {
          ...
          return {
            content: [
              { type: 'text', text: `Error creating payment token: ${getErrorMessage(error)}` }
            ],
          }
        }
      }
    ...
    server.tool(
      ...
      async ({ buyerTag, sellerServiceId, amount }) => {
        try {
          ...
          return {
            content: [
              { type: 'text', text: `Error creating KYA + Payment token: ${getErrorMessage(error)}` }
            ],
          }
        }
      }
    ...
    server.tool(
      ...
      async () => {
        try {
          ...
          return {
            content: [
              { type: 'text', text: `Error getting services: ${getErrorMessage(error)}` }
            ],
          }
        }
      }
    ...
    server.tool(
      ...
      async () => {
        try {
          ...
          return {
            content: [
              { type: 'text', text: `Error getting service tags: ${getErrorMessage(error)}` }
            ],
          }
        }
      }
    ...
    server.tool(
      ...
      async ({ tags }) => {
        try {
          ...
          return {
            content: [
              { type: 'text', text: `Error searching services by tags: ${getErrorMessage(error)}` }
            ],
          }
        }
      }
    ...
    server.tool(
      ...
      async () => {
        try {
          ...
          return {
            content: [
              { type: 'text', text: `Error getting wallet balance: ${getErrorMessage(error)}` }
            ],
          }
        }
      }
    ...
    server.tool(
      ...
      async ({ tokenId }) => {
        try {
          ...
          return {
            content: [
              { type: 'text', text: `Error getting token charges: ${getErrorMessage(error)}` }
            ],
          }
        }
      }
    ...
    server.tool(
      ...
      async ({ token }) => {
        try {
          ...
          return {
            content: [
              { type: 'text', text: `Error introspecting token: ${getErrorMessage(error)}` }
            ],
          }
        }
      }
    ...
    server.tool(
      ...
      'Search for MCP-compatible services in the Skyfire marketplace',
      {
        ...
        tags: z.array(z.string()).optional().describe('Optional tags to filter MCP services'),
      },
      async ({ query, tags }) => {
        try {
          ...
          return {
            content: [
              { type: 'text', text: `Error searching MCP services: ${getErrorMessage(error)}` }
            ],
          }
        }
      }
    ...
    server.tool(
      ...
      {
        serviceId: z.string().describe('Service ID from Skyfire marketplace (if connecting via Skyfire)'),
        // url: z.string().optional().describe('Direct URL to MCP service (for local/development services)'),
        alias: z.string().optional().describe('Optional alias for the service (defaults to service ID or URL)')
      },
      async ({ serviceId, alias }, extra) => {
        try {
          if (!serviceId) {
            throw new Error('Either serviceId (for Skyfire services) or url (for direct connections) must be provided')
          }

          let mcpServerUrl: string
          let serviceName: string
          let serviceDescription: string
          const serviceAlias = alias || serviceId || new URL(url!).hostname
          ...
          try {
            ...
            console.log(`[MCP Connect] Token exchange successful for MCP service: ${targetAudience}`)
            
          } catch (tokenExchangeError) {
            console.warn(`[MCP Connect] Token exchange failed: ${getErrorMessage(tokenExchangeError)}`)
            throw new Error(`Token exchange failed for MCP service ${targetAudience}: ${getErrorMessage(tokenExchangeError)}`)
          }
          ...
          for (const tool of tools) {
            ...
            const success = await registerDynamicTool(toolName, tool, proxyClient)
            if (success) {
          ...
          return {
            content: [{
              type: 'text',
              text: `Error connecting to MCP service: ${getErrorMessage(error)}`
            }]
          }
        }
      }
      ...
      async ({ alias }) => {
        try {
          ...
          return {
            content: [{
              type: 'text',
              text: `Error disconnecting from MCP service: ${getErrorMessage(error)}`
            }]
          }
        }
      }
    ...
    server.tool(
      ...
      async () => {
        try {
          const connections = Array.from(connectedServices.entries()).map(([alias, client]) => ({
            alias,
...
app.get('/', (c) => {
  return c.json({
    message: 'Skyfire Buyer Service MCP Server - Complete Integration with Auth0',
    endpoints: {
      mcp: '/mcp',
      description: 'MCP server with Skyfire buyer service tools and math operations',
    },
    tools: [
